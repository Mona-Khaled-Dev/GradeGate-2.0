...
if (isset($_SESSION['user_id'])) {
    header('Location: ' . ($_SESSION['role'] == 'student' ? 'student.php' : 'teacher.php')); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode == 'register') {
    ...
    $p = $_POST['password'];
    $r = $_POST['role'];
    if ($u == '' || $f == '' || $p == '') { $error = 'All fields required.'; }
    else {
        $check = mysqli_query($conn, "SELECT id FROM users WHERE username='$u'");
        if (mysqli_num_rows($check) > 0) { $error = 'Username taken.'; }
        else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $mode == 'login') {
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        ...
        mysqli_close($conn);
        header('Location: ' . ($user['role'] == 'student' ? 'student.php' : 'teacher.php')); exit;
