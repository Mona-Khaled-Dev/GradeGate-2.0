...
if (isset($_GET['view']) && $_GET['view'] == 'grades') {
    $grades = []; $avg = [];
    $res = mysqli_query($conn, "SELECT s.name AS sub, a.file_name, a.upload_date, a.grade, a.comment
                                FROM assignments a JOIN subjects s ON a.subject_id=s.id
                                WHERE a.student_id=$sid ORDER BY s.name, a.upload_date DESC");
    while ($r = mysqli_fetch_assoc($res)) $grades[] = $r;
    $res2 = mysqli_query($conn, "SELECT s.name, AVG(a.grade) AS av
                                 FROM assignments a JOIN subjects s ON a.subject_id=s.id
                                 WHERE a.student_id=$sid AND a.grade IS NOT NULL GROUP BY s.name");
    while ($r = mysqli_fetch_assoc($res2)) $avg[$r['name']] = round($r['av'],2);

    $avg_html = '';
    if (!empty($avg)) {
        $avg_html = '<h3>Subject Averages</h3><ul>';
        foreach ($avg as $k => $v) $avg_html .= "<li><strong>$k:</strong> $v%</li>";
        $avg_html .= '</ul>';
    }

    $grades_table = '';
    if (empty($grades)) {
        $grades_table = '<p>No grades yet.</p>';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enroll_self'])) {
    ...
    if (mysqli_num_rows($check) > 0) $enroll_msg = 'Already enrolled.';
        ...
        mysqli_query($conn, "INSERT INTO enrollments (student_id, subject_id) VALUES ($sid, $sub_id)");
        $enroll_msg = 'Enrolled!';
    }
}
...
while ($r = mysqli_fetch_assoc($all)) {
    ...
    while ($e = mysqli_fetch_assoc($res)) if ($e['id'] == $r['id']) $found = true;
